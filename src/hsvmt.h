...
class Hsvmt : public Translator {
    private :
        ...
        void store16 (const Variable & mem, const Variable & address, const Variable & value, uint64_t mem_address);
    ...
    public :
        Hsvmt();
        bool translate (uint8_t * data, uint32_t size, uint64_t address, uint64_t mem_address);

        uint16_t size ();

        void setmem (uint8_t * mem, size_t size, uint64_t address);
};
